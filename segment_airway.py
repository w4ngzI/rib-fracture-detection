...
def segment_airway(params, I, I_affine, Mlung, index):
	...
	Mlung[Maw > 0] = 0
	# np.save('./result/RibFrac' + str(index) + '_aw.npy', Maw)
	# np.save('./result/RibFrac' + str(index) + '_lung.npy', Mlung)
	nib.Nifti1Image(Maw, I_affine).to_filename('./result/RibFrac' + str(index) + '_aw.nii.gz')
	nib.Nifti1Image(Mlung, I_affine).to_filename('./result/RibFrac' + str(index) + '_lung.nii.gz')
	return Mlung, Maw
